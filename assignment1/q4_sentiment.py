...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    sentence = b_decode(sentence)
    indices = [tokens[word] for word in sentence]
    # indices = tokens[sentence]
    vectors = wordVectors[indices]
    sentVector = np.mean(vectors, axis = 0)
    ### END YOUR CODE
...
def chooseBestModel(results):
    ...
    ### YOUR CODE HERE
    dev_acc = [result['dev'] for result in results]
    best_id = dev_acc.index(max(dev_acc))
    bestResult = results[best_id]
    ### END YOUR CODE
...
def plotRegVsAccuracy(regValues, results, filename):
    ...
    acc_train = [x["train"] for x in results]
    acc_dev = [x["dev"] for x in results]
    plt.figure()
    plt.plot(regValues, acc_train)
    plt.plot(regValues, acc_dev)
    plt.xscale('log')
...
def outputPredictions(dataset, features, labels, clf, filename):
    ...
    with open(filename, "w") as f:
        ...
        for i in range(len(dataset)):
            # print(dataset[i][0])
            # dataset[i][0] = b_decode(list(dataset[i][0]))
            temp = list(dataset[i][0])
            temp = b_decode(temp)
            f.write("%d\t%d\t%s\n" % (
                labels[i], pred[i], " ".join(temp)))
...
def main(args):
    ...
    words[nWords - 1] = words[nWords - 1].encode()
    words_decoded = b_decode(words)
    for i in range((nWords) - 1):
        tokens[words_decoded[i]] = tokens.pop(words[i])
    tokens['UNK'] = t
    # print(tokens)
